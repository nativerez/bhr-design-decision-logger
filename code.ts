...
async function initializePlugin() {
  try {
    ...
    const savedTheme = await figma.clientStorage.getAsync('ui-theme');
...
async function getOrCreateDesignDecisionsPage(): Promise<PageNode> {
  ...
  let existingPage = figma.root.children.find(page => page.name === 'Design Decisions') as PageNode | undefined;
...
figma.ui.onmessage = async (msg) => {
  ...
  switch (msg.type) {
    ...
    case 'validate-nodes': {
      try {
        ...
        for (const decision of decisions) {
          if (decision.nodeId) {
            try {
              const node = await figma.getNodeByIdAsync(decision.nodeId);
    ...
    case 'set-theme':
      await figma.clientStorage.setAsync('ui-theme', msg.theme);
      break;
